...
class GUI2:
    ...
    def __init__(self):
        ...
        # Model/BLEU/model parameter GUI elements
        ...
        self.translate_button = tk.Button(self.main_frame, text="Translate", font=("Times New Roman", 12), command=self.translate)
    ...
    def api_call(self) -> None:
        ...
        if response.status_code == 200:
            ...
            if self.references:
                if is_cjk(code) == "ZH":
                    self.display_bleu(bleu(code, self.references, tokenize="zh").score)
                elif is_cjk(code) == "JA":
                    self.display_bleu(bleu(code, self.references, tokenize="ja-mecab").score)
                elif is_cjk(code) == "KO":
                    self.display_bleu(bleu(code, self.references, tokenize="ko-mecab").score)
                else:
                    self.display_bleu(bleu(code, self.references).score)
            else:
